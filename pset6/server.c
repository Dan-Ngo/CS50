...
char* indexes(const char* path)
{
    char* buffer = malloc((strlen(path) + strlen("/index.html\n")));
    if (buffer == NULL)
    {
        return NULL;
    }
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    *length = 0;
    int c = fgetc(file);
    BYTE* buffer = malloc(sizeof(char));
    if (buffer == NULL)
    {
        return false;
    }
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    //Handles requesttarget(abs-path) checking
    char* SP = strchr(line, '/');
    
    if (strchr(SP, '"') != NULL || strchr(strchr(SP, ' ') + 1, ' ') != NULL)
    {
        error(400);
        return false;
    }
    //Put strlen of requesttarget into size
    int size = (strlen(strchr(line, ' ')) - strlen(strchr(SP, ' ')));
    char* requesttarget = malloc(size);
    if (requesttarget == NULL)
    {
        return false;
    }
    strncpy(requesttarget, SP, size);
    requesttarget[size - 1] = '\0';
    strcpy(query, "");
    
    
    for (int i = 0; requesttarget[i]; i++)
    {
        if (requesttarget[i] == '?')
        {
            strcpy(query, &requesttarget[i + 1]);
            requesttarget = realloc(requesttarget, i);
            requesttarget[i] = '\0';
        }
    }
    ...
    SP = strchr(SP, ' ');
    SP++;
    // Requesttarget Debugging
        // printf("requesttarget: %s\n", requesttarget);
        // printf("SP: %s\n", SP);
    
    char http[] = "HTTP/1.1\0";
    printf("http[]: %s\n", http);
    for (int i = 0; i < 8; i++)
    {
        http[i] = *SP++;
